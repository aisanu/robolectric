...
public class ShadowApplicationTest {
  ...
  public void checkPermission_shouldTrackGrantedAndDeniedPermissions() throws Exception {
    shadowOf(context).grantPermissions("foo", "bar");
    shadowOf(context).denyPermissions("foo", "qux");
    assertThat(context.checkPermission("foo", -1, -1)).isEqualTo(PERMISSION_DENIED);
    assertThat(context.checkPermission("bar", -1, -1)).isEqualTo(PERMISSION_GRANTED);
    assertThat(context.checkPermission("baz", -1, -1)).isEqualTo(PERMISSION_DENIED);
    assertThat(context.checkPermission("qux", -1, -1)).isEqualTo(PERMISSION_DENIED);
  }
